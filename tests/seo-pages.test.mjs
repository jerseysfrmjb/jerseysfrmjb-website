...
assert.match(barcelonaHtml, /secure (?:Shopify|website) checkout/i);
assert.match(barcelonaHtml, /Available jerseys can be purchased directly through JerseysFrmJB using secure Shopify checkout/);
assert.doesNotMatch(barcelonaHtml, /Payment and checkout are completed on Depop or eBay/);
assert.match(barcelonaHtml, /styles\.css\?v=checkout-promo-1/);
assert.match(barcelonaHtml, /storefront\.js\?v=checkout-promo-1/);
